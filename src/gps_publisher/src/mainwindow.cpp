...
{
    ui->setupUi(this);
    QString settingsFilePath = QDir::homePath() + "/Projects/ROS/src/gps_publisher/config/gps_config.txt";
...
{
    QString settingsDir = QDir::homePath() + "/Projects/ROS/src/gps_publisher/config";
    ...
    if(startGpsProcess->state() == QProcess::NotRunning){
        ...
        QString command = QString("bash -c \"source /home/phenobot4/Projects/ROS/devel/setup.bash && roslaunch gps_publisher start_gps.launch com_port:=%1 baud:=%2\"")
                          .arg(com_port)
                          .arg(baud);
